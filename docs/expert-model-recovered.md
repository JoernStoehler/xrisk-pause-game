# Recovered Expert Model

Status: recovered draft from branch `stream/expert-model-extraction-recovered`.
This is close-source expert-model material from Jörn's 2026-05-11 chat, plus
compact Codex synthesis where noted. It is not final card text, not public
wording, and not a settled ontology.

Use this file as the fastest recovered expert-model surface. Use
`docs/expert-model-chat-excerpts.md` when exact wording matters. Treat all
synthesis here as fallible until checked against the raw excerpts and Jörn.

## Source Layers

1. Raw source:
   `docs/expert-model-chat-excerpts.md`
2. Structured synthesis:
   not imported; old branch-only artifact
3. Recovery and process state:
   not imported; summarized in this file
4. Draft continuation prompt:
   not imported; compute-control points below are draft-only
...
2. The named `/tmp/expert-model-*` scratch files.
3. The named raw session logs from 2026-05-11 in this environment.
...
Source status: mostly draft from an old branch-only next-question artifact, not
yet answered by Jörn as `CMP-*`.
...
Current planned question from the old branch: `CMP-Q01`, compute control and
verification.

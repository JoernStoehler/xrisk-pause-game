...
3. Keep `tasks/MAP.md` as the session-start roadmap instead of `ROADMAP.md`.
4. Keep `research-topic` and `write-cards` as TODO placeholders until Jörn
   writes or approves them.
5. Add missing task coverage for deployment/generated artifacts and test
...
- Gives us `scripts/codex-worktree.sh`, `scripts/toc.sh`, task conventions, and
  generic quality/worktree/postmortem skills.
...
Observation: deployment is not currently first-class in `AGENTS.md` or
`tasks/MAP.md`. It does not need much text, but it should be discoverable.
...
- Whether `tasks/MAP.md` fully replaces `ROADMAP.md`.
- Whether the skill TODO placeholders are acceptable for merge.
- Whether to add a deploy/release task bundle or just mention deployment in
  `tasks/app.md`.
- Whether `project-quality` is a useful separate skill or should be folded into
  `harness-engineering` and `tasks/README.md`.
- Whether `scripts/codex-worktree.sh` should create branches from `HEAD` by
  default, as adapted here, or require explicit existing branch names.
...
4. Decide TODO-vs-interim-body for `write-cards` and `research-topic`.
5. Add a source map row for `.github/workflows/deploy.yml` and `wrangler.toml`
   in `AGENTS.md`.
...
## Self-Contained Questions For Jörn
